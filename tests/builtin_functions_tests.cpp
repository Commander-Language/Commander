...
TEST(GENERALTESTS, intToIntTest) {
    ...
    int64_t newValue = Function::parseAsType<int64_t, int64_t>(originalValue);
    EXPECT_EQ(newValue, originalValue);
}

TEST(GENERALTESTS, boolToBoolTest) {
    bool originalValue = true;
    bool newValue = Function::parseAsType<bool, bool>(originalValue);
...
TEST(GENERALTESTS, floatToFloatTest) {
    ...
    double newValue = Function::parseAsType<double, double>(originalValue);
    EXPECT_EQ(newValue, originalValue);
}

TEST(INTTESTS, intToBoolTest) {
    int64_t originalValue = 8;
    bool newValue = Function::parseAsType<int64_t, bool>(originalValue);
    EXPECT_TRUE(newValue);
    originalValue = 0;
    newValue = Function::parseAsType<int64_t, bool>(originalValue);
    EXPECT_FALSE(newValue);
}

TEST(INTTESTS, intToFloatTest) {
    int64_t originalValue = 8;
    double newValue = Function::parseAsType<int64_t, double>(originalValue);
    EXPECT_EQ(8.0, newValue);
}

TEST(FLOATTESTS, floatToBoolTest) {
    double originalValue = 3.14;
    bool newValue = Function::parseAsType<double, bool>(originalValue);
    EXPECT_TRUE(newValue);
    originalValue = 0;
    newValue = Function::parseAsType<double, bool>(originalValue);
    EXPECT_FALSE(newValue);
}

TEST(FLOATTESTS, floatToIntTest) {
    double originalValue = 3.14;
    int64_t newValue = Function::parseAsType<double, int64_t>(originalValue);
    EXPECT_EQ(3, newValue);
}

TEST(STRINGTESTS, boolToStringTest) {
    bool originalValue = true;
    std::string newValue = Function::parseAsString(originalValue);
    EXPECT_EQ("true", newValue);
    originalValue = false;
    newValue = Function::parseAsString(originalValue);
