...
void TypeChecker::setOrUpdateType(std::string variableID, std::string type) { assignedTypes[variableID] = type; }
...
bool TypeChecker::verifyType(std::string variableID, std::string expected) {
    ...
    if (assignedTypes[variableID] == expected)
...
bool TypeChecker::hasVariable(std::string variableID) { return assignedTypes.find(variableID) != assignedTypes.end(); }
