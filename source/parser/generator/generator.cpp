...
namespace Parser {
    ...
    void Generator::generateSource(std::ostream& output) const {
        ...
        const std::string head = _join("\n", {
            ...
            "    }",
            "    std::shared_ptr<LValueNode> castLValue(const std::shared_ptr<ASTNode>& node) {",
            "        return std::reinterpret_pointer_cast<LValueNode>(node);",
            "    }",
            "    std::shared_ptr<BasicCmdNode> castBasicCmd(const std::shared_ptr<ASTNode>& node) {",
            "        return std::reinterpret_pointer_cast<BasicCmdNode>(node);",
            "    }",
            "    std::shared_ptr<PipeCmdNode> castPipeCmd(const std::shared_ptr<ASTNode>& node) {",
            "        return std::reinterpret_pointer_cast<PipeCmdNode>(node);",
            "    }",
            "    std::shared_ptr<AsyncCmdNode> castAsyncCmd(const std::shared_ptr<ASTNode>& node) {",
            "        return std::reinterpret_pointer_cast<AsyncCmdNode>(node);",
            "    }",
            ""
        });
