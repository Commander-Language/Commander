...
namespace Parser {
    ...
    size_t ParseTable::HashGrammarEntry::operator()(const Parser::ParseTable::GrammarEntry& entry) const {
        ...
        if (entry.tokenType.has_value()) {
            hash ^= std::hash<int>()(static_cast<int>(*entry.tokenType)) + (sizeof(size_t) >= 8 ? 0x517cc1b727220a95 : 0x9e3779b9) + (hash << 6) + (hash >> 2);
        }
        if (entry.nodeType.has_value()) {
            hash ^= std::hash<int>()(static_cast<int>(*entry.nodeType)) + (sizeof(size_t) >= 8 ? 0x517cc1b727220a95 : 0x9e3779b9) + (hash << 6) + (hash >> 2);
        }
        return hash;
    }
    ...
    size_t ParseTable::_combineHashes(size_t hash1, size_t hash2) {
        // Implementation inspired by Yakk-Adam Nevraumont's post at https://stackoverflow.com/questions/5889238/why-is-xor-the-default-way-to-combine-hashes
